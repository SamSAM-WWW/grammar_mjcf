...
from uni.utils import Transform
import numpy as np
from typing import Optional
from scipy.spatial.transform import Rotation
from uni.xml_parser.joints import Joint
from uni.xml_parser.geoms import Geom, Sphere, Capsule, Cylinder, Box
import yourdfpy
    ...
    def from_path(path):
        ...
        for key, link in link_map.items():
            ...
            for geom in collisions:
                ...
                if geometry.box:
                    ...
                    g = Box(pos=xyz, quat=quat, size=size)
                elif geometry.cylinder:
                    radius = geometry.cylinder.radius
                    length = geometry.cylinder.length
                    p1 = np.array([-length/2, 0, 0])
                    p2 = np.array([length/2, 0, 0])
                    p1 = r.apply(p1) + xyz
                    p2 = r.apply(p2) + xyz
                    g = Capsule(p1=p1, p2=p2 ,size=radius)
                elif geometry.sphere:
                    radius = geometry.sphere.radius
                    g = Sphere(pos=xyz,size=radius)
                else:
                    raise NotImplementedError
                bodies[key].add_geom(g)
        ...
        for key, joint in joint_map.items():
            ...
            if joint_type == 'fixed':
                ...
                limits = [joint.limit.lower / np.pi * 180, joint.limit.upper / np.pi * 180]
                damping = float(joint.dynamics.damping)
                stiffness = float(joint.dynamics.friction)
                name = joint.name
                _joint = Joint(name=name, axis=axis, limits=limits, damping=damping, stiffness=stiffness, gear=effort, velocity=velocity)
                bodies[child].add_joint(_joint)
            bodies[child].pos = xyz
            bodies[child].quat = quat
            bodies[parent].bodies.append(bodies[child])

        assert len(root) == 1
        root = bodies[root.pop()]
        m = URDFModel()
        m.root = root
        return m
...
class XMLModel:
    ...
    def from_path(path):
        m = XMLModel()
        xml = ET.parse(path)
        m.root = Body.from_xml(
            xml.getroot().find("worldbody").find("body"), xml.getroot().find("actuator")
        )
        return m
    ...
    def write(self, path):
        ...
        defaults = ET.SubElement(xml, "default", {"class": "decor"})
        ET.SubElement(defaults, "site", attrib={"type": "box", "rgba": "0.5 1 0.5 1"})
        assets = ET.SubElement(xml, "asset")
        ET.SubElement(
            assets, "material", attrib={"name": "default", "rgba": "0.97 0.38 0.06 1.0"}
        )
        ET.SubElement(
            assets, "material", attrib={"name": "red", "rgba": "1.0 0.0 0.0 1.0"}
        )
        ET.SubElement(
            assets, "material", attrib={"name": "green", "rgba": "0.0 1.0 0.0 1.0"}
        )
        ET.SubElement(
            assets, "material", attrib={"name": "blue", "rgba": "0.0 0.0 1.0 1.0"}
        )
        ET.SubElement(
            assets, "material", attrib={"name": "white", "rgba": "1.0 1.0 1.0 1.0"}
        )
        ET.SubElement(
            assets, "material", attrib={"name": "black", "rgba": "0.0 0.0 0.0 1.0"}
        )
...
class Body:
    def __init__(self, name=None, pos=[0.0, 0.0, 0.0], quat=[0.0, 0.0, 0.0, 1.0]):
        self.name = name
    ...
    def add_body(self, name=None, pos=[0.0, 0.0, 0.0], quat=[0.0, 0.0, 0.0, 1.0]):
    ...
    def to_xml(
        ...
        for j in self.joints:
            joint_name = (
                j.name if j.name is not None else name_manager.get_name("joint")
            )
            if j.limits is None:
                ET.SubElement(
                    xml,
    ...
    def from_xml(xml: ET.Element, actuator: ET.Element):
        ...
        for g in xml.findall("geom"):
            if g.attrib["type"] == "capsule":
                attr = _parse_available(
                    g.attrib, ["fromto", "size", "density", "friction"]
                )
                ...
                attr = _parse_available(
                    g.attrib, ["pos", "size", "density", "friction"]
                )
                body.add_geom(Sphere(**attr))
            elif g.attrib["type"] == "cylinder":
                attr = _parse_available(
                    g.attrib, ["fromto", "size", "density", "friction"]
                )
                attr["p1"] = attr["fromto"][:3]
                attr["p2"] = attr["fromto"][3:]
                del attr["fromto"]
                body.add_geom(Cylinder(**attr))
            elif g.attrib["type"] == "box":
                attr = _parse_available(
                    g.attrib, ["pos", "quat", "size", "density", "friction"]
                )
                attr["quat"] = attr["quat"][1:] + attr["quat"][0:1]
                body.add_geom(Box(**attr))
            else:
        ...
        for j in xml.findall("joint"):
            ...
            if motor is None:
                raise RuntimeError(
                    f'Unable to find a motor associated with joint {j.attrib["name"]}'
                )
            attr = _parse_available(
                j.attrib,
                ["pos", "axis", "range", "damping", "armature", "stiffness", "name"],
            )
    ...
    def get_lowest_point(self, transform=None):
        ...
        for joint in self.joints:
            ...
            angle *= np.pi / 180.0  # convert to radians
    ...
    def to_urdf(
        ...
        for g in self.geoms:
            ...
            for element in [visual, collision]:
                if isinstance(g, Capsule):
                    ...
                    origin = [(x1 + x2) / 2.0 for x1, x2 in zip(g.p1, g.p2)]
                    ...
                    if np.allclose(rot_axis, 0.0):
                        ...
                        rpy = (
                            Rotation.from_rotvec(angle * rot_axis)
                            .as_euler("xyz")
                            .tolist()
                        )
                    ET.SubElement(
                    ...
                    ET.SubElement(
                        geom,
                        "cylinder",
                        attrib={"radius": str(g.size), "length": str(length)},
                    )
                elif isinstance(g, Cylinder):
                    # adjust ori so Cylinder axis is aligned with z axis
                    origin = [(x1 + x2) / 2.0 for x1, x2 in zip(g.p1, g.p2)]
                    geom_axis = np.array([x2 - x1 for x1, x2 in zip(g.p1, g.p2)])
                    length = np.linalg.norm(geom_axis)
                    geom_axis /= length
        ...
        if len(self.joints) == 1:
            ...
            joint = ET.SubElement(
                xml, "joint", attrib={"name": joint_name, "type": "revolute"}
            )
            rpy = Rotation.from_quat(self.quat).as_euler("xyz").tolist()
            limits = [np.pi * lim / 180.0 for lim in j.limits]
            ET.SubElement(
                joint, "origin", attrib={"xyz": _to_str(self.pos), "rpy": _to_str(rpy)}
            )
            ...
            ET.SubElement(
                mat,
                "color",
                attrib={"rgba": " ".join([str(x) for x in j.axis]) + " 1.0"},
            )
            ...
            for i, j in enumerate(self.joints):
                joint = ET.SubElement(
                    xml,
                    "joint",
                    attrib={"name": joint_name + f"_{i}", "type": "revolute"},
                )
                rpy = Rotation.from_quat(self.quat).as_euler("xyz").tolist()
                limits = [np.pi * lim / 180.0 for lim in j.limits]
                ET.SubElement(
                    joint,
                    "origin",
                    attrib={"xyz": _to_str(self.pos), "rpy": _to_str(rpy)},
                )
                ET.SubElement(joint, "axis", attrib={"xyz": _to_str(j.axis)})
                ET.SubElement(
                    joint,
...
if __name__ == "__main__":
    ...
    model = XMLModel.from_path("../../xmls/000000.xml")
    model.adjust_root()
    model.write("../../xmls/test.xml")
