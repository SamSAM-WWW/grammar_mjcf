...
from uni.envs.isaac_env import IsaacEnv
from uni.scenes import MixedXMLScene
from uni.utils import torch_jit_utils as tjit
from uni.tasks.state_collection import StateCollection
from uni.tasks.task_common import TaskCommon
import torch
import gin
import gym
import numpy as np
from uni.utils.misc import pack_gym_space
import pytorch3d.transforms as transforms
from uni.envs.isaac_env import Task
...
class MoveToTarget(Task):
    ...
    def observe_fn(self, tensor_api, actions):
        # print(tensor_api.actor_root_state.position[:, :])
        ...
        # print(height-self.scene.terrain_indexer(self.scene.tensor_api))
        # obs['terrain'] = {'obs': torch.clip(height-self.scene.terrain_indexer(self.scene.tensor_api), 0., 5.)}
        obs['terrain'] = {'obs': height-self.scene.terrain_indexer(self.scene.tensor_api),}
        return obs

    def done_fn(self, tensor_api, actions):
        height = tensor_api.actor_root_state.position[self.scene.actor_inds, 2:3]
        terrain_height = height - self.scene.terrain_indexer(self.scene.tensor_api)
        root_rel_height = torch.max(terrain_height, dim=1)[0]
        return self.task_common.compute_termination_common(root_rel_height)

    def reward_fn(self, tensor_api, actions):
        dof_state, wheel_mask = self.state_collection.get_dof_state()
        # print("dof_state: ", dof_state.shape, " wheel_mask: ", wheel_mask.shape)
        # print("actions: ", actions.shape)
        root_vel = tensor_api.actor_root_state.linvel[self.scene.actor_inds]
        self._update_root_ori(tensor_api.actor_root_state.orientation[self.scene.actor_inds])
        ...
        self._reward[:] = self.task_common.compute_reward_common(
            ...
            root_vel=root_vel,
            root_rel_height=root_rel_height,
        )
