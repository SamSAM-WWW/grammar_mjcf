...
class CuriculumTerrain()
    ...
    def __init__(self, num_robots, type, gym gymapi.Gym, sim gymapi.Sim, device torch.device)
        ...
        self.border = int(self.border_sizeself.horizontal_scale)
    ...
    def get_heights(self, tensor_api)
        ...
        self._quat[..., 01] = tensor_api.actor_root_state.orientation[self.actor_inds, 34]
        self._quat[..., 34] = tensor_api.actor_root_state.orientation[self.actor_inds, 23]
        ...
        points = (pointsself.horizontal_scale).long()
        px = points[, , 0].view(-1)
        py = points[, , 1].view(-1)
    ...
    def add_terrain_to_map(self, terrain, row, col)
        ...
        self.height_field_raw[start_x end_x, start_yend_y] = terrain.height_field_raw

        env_origin_x = (i + 0.5)  self.env_length
        env_origin_y = (j + 0.5)  self.env_width
        x1 = int((self.env_length2. - 1)  terrain.horizontal_scale)
        x2 = int((self.env_length2. + 1)  terrain.horizontal_scale)
        y1 = int((self.env_width2. - 1)  terrain.horizontal_scale)
        y2 = int((self.env_width2. + 1)  terrain.horizontal_scale)
        env_origin_z = np.max(terrain.height_field_raw[x1x2, y1y2])terrain.vertical_scale
...
def pit_terrain(terrain, depth, platform_size=1.)
    ...
    terrain.height_field_raw[x1x2, y1y2] = -depth