...
from .terrain import TerrainIndexer, CuriculumTerrain
...
def get_init_z(paths):
    z = []
    for path in paths:
        xml = XMLModel.from_path(path)
        xml.adjust_root()
        z.append(xml.get_height())
    return z

def get_init_quat(paths):
    quat = []
    for path in paths:
        tree = ET.parse(path)
        w, a, b, c = tree.getroot().find('worldbody').find('body').get('quat').split()
        w, a, b, c = float(w), float(a), float(b), float(c)
        quat.append([a, b, c, w])
    return quat
...
class MixedXMLScene(IsaacScene):
    ...
    def create_scene(self):
        ...
        self.markers = []
        if self.load_urdf:
            self.init_z = [-xml.root.get_lowest_point() for xml in self.xml_models]
            self.init_quat = None
        else:
            self.init_z = get_init_z([os.path.join(self.asset_root, f) for f in self.asset_files])
            self.init_quat = get_init_quat([os.path.join(self.asset_root, f) for f in self.asset_files])
        if self.load_urdf:
        ...
        for i in range(self.num_envs):
            ...
            if self.init_quat:
                quat = self.init_quat[i % len(self.init_quat)]
    ...
    def reset(self, env_ids: Optional[torch.Tensor] = None):
        ...
        if self.homogeneous_envs:
            ...
            for env_id in env_ids:
                ...
                # if self.init_quat:
                #     quat = self.init_quat[env_id % len(self.init_quat)]
                #     initial_root_state[3:] = torch.tensor(quat, requires_grad=False).to(initial_root_state.device)
