...
import numpy as np
import torch
from uni.xml_parser.geoms import Sphere, Capsule, Cylinder, Box
...
class JointObservation():
    ...
    def add_joint(self, env_id: int, frame_id: int, global_frame_id: int, joint_id: int,
                  ...
                  data: torch.Tensor, type: int):
        # type (0: normal, 1: wheel)
        self.pos.append(pos)
    ...
    def finalize(self, num_envs, max_bodies: int, device='cpu'):
        ...
        self.types = torch.tensor(self.types, dtype=torch.long, device=device)
        self.data = torch.stack(self.data).float().to(device)
        self.device = device
        self.num_envs = num_envs
        self.max_bodies = max_bodies
        ...
        # self._wheel_inds = self._inds[self.types == 1]

        body_id_order = torch.arange(0, self.max_bodies)

        _type = torch.max(self.types)
        # self._type_obs.fill_(_type)
        self._type_obs.view(-1, 1)[self._inds] = _type
        ...
        for i in range(self.num_envs):
            ...
            for j in torch.unique(self.frame_ids[env_mask]):
                ...
                if n_joints == 1:
                    self._wheel_mask[i, j, :1] = self.types[joint_mask][0] == 1
                    self._mask[i, j, :n_joints] = self.types[joint_mask][0] == 0
    ...
    def get_dof_state(self, dofs):
        ...
        return self._obs[..., :2], self._wheel_mask
    ...
    def get_obs_space(self):
        return {'obs': gym.spaces.Box(-np.inf, np.inf, (self.n_obs,)),
                ...
                'mask': gym.spaces.Discrete(2),
                'wheel_mask': gym.spaces.Discrete(2),
...
class StateCollection():
                ...
                for geom_ind, geom in enumerate(body.geoms):
                    if isinstance(geom, Capsule):
                        ...
                        self.geoms.add_geom(
                                ...
                                global_frame_id=global_body_ind,
                                geom_id=geom_ind,
                                points=torch.tensor([p1, p2]),
                                data=torch.tensor([geom.get_size(), geom.density, *geom.friction])
                        )
                    else:
    ...
    def get_root_obs(self, remove_xy=False):
        # print(self.api.actor_root_state.state[self.scene.actor_inds].clone())
        if remove_xy:
...
if __name__ == '__main__':
    scene = MixedXMLScene(num_envs=1, homogeneous_envs=False)
    scene.set_asset_root('../../xmls')
    scene.create_scene()
    scene.reset()
    ...
    print(joint_obs['mask'])
    print(joint_obs['wheel_mask'])
    print(joint_obs['parents'])
