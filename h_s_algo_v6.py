...
load_V_path = '/home/ps/pan1/files/Sam/grammar_mjcf-master/mjcf_model/2024-07-04_21-43-44/V_gnn_inpro.pt'
...
def update_controller(uni):
    return uni.simulate(2000000)

def update_controller_2(uni,xml_name):
    return uni.simulate_2(xml_name,3000000)
...
def get_reward(uni,eps,xml_name):
    ...
    if use_or_train < eps:
        ...
        reward_dict = update_controller_2(uni,xml_name)
...
def search_algo():
    ...
    if load_controller_path is None:
        #生成100个初始模型 并且预训练控制器
        for i in range(20):
        ...
        uni = UniSimulator(new_folder_path,'/home/ps/pan1/files/Sam/grammar_mjcf-master/my_config/grammar.gin')
        print_ok("uni initialized")

        pre_train(uni)
        
        print("pre-train finished!")
    else:
        print("loading old V, and old controller")
        #生成1个初始模型 并且不训练控制器
        for i in range(1):
            filename = 'xmlrobot_pre_' + str(i)
            R = result_R(filename)
            ...
            tree.write(xml_file_path)
        uni = UniSimulator(new_folder_path,'/home/ps/pan1/files/Sam/grammar_mjcf-master/my_config/grammar.gin')
        copy_dir(src_path=load_controller_path,target_path='/home/ps/pan1/files/ypf/exps/grammar/ckpts')
    ...
    for epoch in range(num_iterations):
        ...
        for num in range(num_samples):
            ...
            po = random.random()
            if po < eps:
                best_state = random.choice(samples)
                predicted_value = predict_gnn(V,best_state)
                selected_design, selected_reward = best_state, predicted_value
    ...
    writer.close()
    UniSimulator.visual(new_folder_path,'/home/ps/pan1/files/Sam/grammar_mjcf-master/my_config/grammar.gin')
