...
from uni.utils import nest, misc
from uni.rl.modules import NestedRunningNorm
import gym
import torch
...
class CoOptObsNormWrapper(gym.Wrapper):
    ...
    def update_obs_norm(self, obs):
        ...
        assert sorted(list(obs.keys())) == ['geom', 'inertia', 'joint', 'root', 'task', 'terrain', 'upper_terrain']
        ...
        for k in ['root', 'task', 'terrain', 'upper_terrain']:
