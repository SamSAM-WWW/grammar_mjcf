...
class DAG(object):
    ...
    def add_edge(self, ind_node, dep_node, graph=None, info = None):
        ...
        if is_valid:
            ...
            raise Exception()
        if info: # 添加边信息
            if ind_node not in self.edge_info : # 该字典为空 
                self.edge_info[ind_node] = dict()
            self.edge_info[ind_node][dep_node] = info
    ...
    def reset_graph(self):
        ...
        self.graph = OrderedDict() # 有序字典
        self.node_info = OrderedDict() # 存储节点信息
        # node_info: {'node id': info }
        self.edge_info = OrderedDict()
        # edge_info: {'head node id': {tail node id : edge info} }
